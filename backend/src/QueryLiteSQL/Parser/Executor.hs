...
import QueryLiteSQL.Parser.SQL (SQLQuery(..), SQLSelect(..), SQLWhere(..), Condition(..), BinaryOp(..), Value(..))
...
selectColumns columns (Object obj) =
    let selectedFields = mapMaybe (\col -> let key = Key.fromText col in 
                                   (key,) <$> HM.lookup key obj) 
                         [Key.fromText col | col <- columns]
...
evaluateBinaryOp Equals (SQL.ColumnRef colRef) row = 
    case extractValue colRef row of
        Just val -> evaluateBinaryOp Equals (toSQLValue val) val
        Nothing -> False
evaluateBinaryOp Equals _ _ = False
...
evaluateBinaryOp LessThan (SQL.ColumnRef colRef) row =
    case extractValue colRef row of
        Just val -> evaluateBinaryOp LessThan (toSQLValue val) val
        Nothing -> False
...
evaluateBinaryOp GreaterThan (SQL.ColumnRef colRef) row =
    case extractValue colRef row of
        Just val -> evaluateBinaryOp GreaterThan (toSQLValue val) val
        Nothing -> False
...
extractValue column (Object obj) = HM.lookup (Key.fromText column) obj
